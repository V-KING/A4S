...
import org.firmata.BytesHelper;
...
public class Firmata {
	...
	private final int ANALOG_MESSAGE = 0xE0; // send data for an analog pin (or
												// PWM)
	...
	private final int SERVO_CONFIG = 0x70; // set max angle, minPulse, maxPulse,
											// freq
	private final int STRING_DATA = 0x71; // a string message with 14-bits per
											// char
	...
	private final int I2C_CONFIG = 0x78; // config I2C settings such as delay
											// times and power pins
	private final int EXTENDED_ANALOG = 0x6F; // analog write (PWM, Servo, etc)
												// to any pin
	private final int PIN_STATE_QUERY = 0x6D; // ask for a pin's current mode
												// and value
	private final int PIN_STATE_RESPONSE = 0x6E; // reply with pin's current
													// mode and value
	private final int CAPABILITY_QUERY = 0x6B; // ask for supported modes and
												// resolution of all pins
	private final int CAPABILITY_RESPONSE = 0x6C; // reply with supported modes
													// and resolution
	private final int ANALOG_MAPPING_QUERY = 0x69; // ask for mapping of analog
													// to pin numbers
	private final int ANALOG_MAPPING_RESPONSE = 0x6A; // reply with mapping info
	private final int REPORT_FIRMWARE = 0x79; // report name and version of the
												// firmware
	private final int SAMPLING_INTERVAL = 0x7A; // set the poll rate of the main
												// loop
	private final int SYSEX_NON_REALTIME = 0x7E; // MIDI Reserved for
	 ...
	 *
	 * @param writer
	 *            an instance of the Firmata.Writer interface
	 */
	...
	// 在arduino段，只有7bit的地址，因为address是byte类型的
	public void i2cRead(int slaveAddr, int cmd, int numBytes) {
		...
		int addrMSB = 0;
		addrMSB = BytesHelper.setBit(addrMSB, 7, false); // {7: always 0}
		addrMSB = BytesHelper.setBit(addrMSB, 6, false); // {6: reserved}
		addrMSB = BytesHelper.setBit(addrMSB, 5, false); // {5: address mode,
															// 1 means 10-bit
															// mode}
		// 4-3 bits are modes
		addrMSB |= 0b00001000; // {4-3: read/write, 00 => write, 01 => read
								// once, 10 => read continuously, 11 => stop
								// reading}
		// 0-2 bits
		// {2-0: slave address MSB in 10-bit mode, not used in 7-bit mode}
		out.write(addrMSB);
		
		sendValueAsTwo7bitBytes((char)cmd);
		sendValueAsTwo7bitBytes((char)numBytes);
//		out.write(0b00100000); //
//		out.write(0);
//		out.write(2);
//		out.write(0);
		out.write(END_SYSEX);
	...
	public void sendString(String data) {
		...
		for (int i = 0; i < data.length(); i++) {
			sendValueAsTwo7bitBytes(temp[i]);
		}
		out.write(END_SYSEX);
	}

	private void sendValueAsTwo7bitBytes(char c) {
		out.write(c & 0B01111111); // LSB
		out.write(c >> 7 & 0B01111111); // MSB
	}
	...
	private void setVersion(int majorVersion, int minorVersion) {
		// System.out.println("version is " + majorVersion + "." +
		// minorVersion);
	...
	private void processSysexMessage() {
		// System.out.print("[ ");
		// for (int i = 0; i < storedInputData.length; i++)
		// System.out.print(storedInputData[i] + " ");
		// System.out.println("]");
	...
	private void storeI2CBytes(int numBytes) {
		...
		for (int i = 0; i < numI2cData; i++) {
			...
			msg.data[i]= i2cInputData[i];
			
			//add Map<String,I2cMessage> to List
			mapS_I2c.put(Integer.toString(addr), msg);
